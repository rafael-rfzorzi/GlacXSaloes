...
class QuartoFrame:
    def quarto_frame(self):
        ...
        self.lbTecnico.configure(text='Atendente')
        ...
        self.botaotec = Button(self.FrameTec,
        ...
        self.botaoAbreOrc = Button(self.FrameTec,
                                   bd=0,
                                   image=self.gravarBt,
                                   command=self.abre_orc)
        self.botaoAbreOrc.place(relx=0.41, y=5,
                                width=47, height=32)

        # Botao Buscar
        self.buscarBt = self.buscarBt.subsample(3, 3)
        self.botaoCarregaOrc = Button(self.FrameTec,
                                      bd=0,
                                      image=self.buscarBt,
                                      command=self.busca_orc)
        self.botaoCarregaOrc.place(relx=0.46, y=5,
                                   width=47, height=32)

        # Botao Alterar
        self.alterarBt = self.alterarBt.subsample(3, 3)
        self.botaoAlteraOrc = Button(self.FrameTec,
                                     bd=0,
                                     image=self.alterarBt,
                                     command=self.altera_orc)
        self.botaoAlteraOrc.place(relx=0.51, y=5,
                                  width=47, height=32)
        ...
        self.descrtotal.configure(text=self.m_Total,
                                  command=self.total_orc)
        self.descrtotal.place(relx=0.75, rely=0.25,
                              relwidth=0.1, relheight=0.6)
